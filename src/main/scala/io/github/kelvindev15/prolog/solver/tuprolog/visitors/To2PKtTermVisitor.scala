...
class To2PKtTermVisitor(scope: KScope = ktEmptyScope)
  ...
  override def visit(struct: Struct): KTerm = struct match
    ...
    case _ =>
      ktStructOf(
        struct.functor.unquotedValue,
        struct.arguments.map(this.visit)*
      )

  override def visit(fact: Fact): KTerm =
    fact.head.map(h => ktFactOf(h.accept(this).asStruct())).get

  override def visit(rule: Rule): KTerm =
    rule.head
      .map(h => ktRuleOf(h.accept(this).asStruct(), rule.body.accept(this)))
      .get

  override def visit(directive: Directive): KTerm = ktDirectiveOf(
    directive.body.accept(this)
  )
