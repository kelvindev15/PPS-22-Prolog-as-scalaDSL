...
trait PrologDSL extends DSLPrologBuiltins with DSLExtensions with DSLVariables with DSLConversions:
  ...
  /** Returns a [[Fact]].
   * 
   * @param fact the struct representing the fact.
   */
   ...
   * @param body1 the first term of the rule's body.
   * @param others the other terms of the rule's body.
   */
  def ruleOf(head: Struct, body1: Term, others: Term*): Rule = Rule(head, others*)
  ...
  /** Returns a term which is a [[Disjunction]] of goals if more than one term is provided. 
   * otherwise it returns the provided term.
   * 
   * @param terms the terms to put in disjunction.
   */
  @targetName("disjunction")
  def ||(terms: Term*): Term = wrap(terms*)(_ or _)

  /** Returns a term which is a [[Conjunction]] of goals if more than one term is provided. 
   * otherwise it returns the provided term.
   * 
   * @param terms the terms to put in conjunction.
   */
   ...
   * 
   * @param terms the terms to put in the list.
   */
  def list(terms: Term*): PrologList = PrologList(terms *)

  /** Returns a [[PrologList]].
   * 
   * @param term the head of the list.
   ...
   * 
   * @param terms the terms that would be before the pipe.
   * @param tail the tail of the list
   */
