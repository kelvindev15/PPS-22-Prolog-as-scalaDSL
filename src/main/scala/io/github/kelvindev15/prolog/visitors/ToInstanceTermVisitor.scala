...
class ToInstanceTermVisitor(substitution: Substitution)
  ...
  override def visit(list: PrologList): Term = PrologList(
    list.linearizedArguments.map(visit)*
  )
  override def visit(variable: Variable): Term =
