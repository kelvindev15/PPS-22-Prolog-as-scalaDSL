...
class TestPrologDSL extends AnyFunSuite with Matchers with PrologDSL:
  ...
  test("Creation of a directive"):
    ...
    d.asInstanceOf[Directive].body shouldBe Struct(
      Atom("op"),
      Constant(1199),
      Atom("xfx"),
      Atom("-->")
  ...
  test("Creation of a simple theory"):
    val t = theory(
      "search" (X, "cons" (X, `__`)),
      "search" (X, "cons" (`__`, Variable("Xs"))) :- "search" (
        X,
        Variable("Xs")
      )
    )
    t should have size 2
    t.head shouldBe Fact(
      Struct(
        Atom("search"),
        Variable("X"),
        Struct(Atom("cons"), Variable("X"), anonymous())
      )
    )
