...
import io.github.kelvindev15.prolog.core.Struct.{
  Directive,
  Fact,
  Indicator,
  Rule
...
class TestTermConversion extends AnyFunSuite with Matchers:
  ...
  test("A rule is a binary struct with :- as a functor"):
    ...
    Rule(head, bodyArgs*).asTerm shouldBe Struct(
      Atom(":-"),
      head,
      Struct(Atom(","), bodyArgs*)
    )

  test("A fact is a binary struct with :- as a functor"):
    val head =
      Struct(Atom("sequence"), Atom("a"), Atom("b"), Atom("c"), Atom("d"))
    Fact(head).asTerm shouldBe Struct(Atom(":-"), head, Atom("true"))

  test("A directive is a unary struct with :- as a functor"):
    Directive(
      Struct(Atom("dynamic"), Indicator(Atom("foo"), Constant.Numeric(2)))
    ).asTerm shouldBe
      Struct(
        Atom(":-"),
        Struct(
          Atom("dynamic"),
          Struct(Atom("/"), Atom("foo"), Constant.Numeric(2))
        )
      )

  test(
    "A conjunction of goals is a binary recursive struct with , as a functor"
  ):
    val functor = Atom(",")
  ...
  test(
    "A disjunction of goals is a binary recursive struct with , as a function"
  ):
    val functor = Atom(";")
    ...
    PrologList(Atom("a"), Atom("b"), Atom("c"), Atom("d")).asTerm shouldBe
      Struct(
        ...
        Struct(
          functor,
          Atom("b"),
          Struct(functor, Atom("c"), Struct(functor, Atom("d"), Atom("[]")))
        )
      )
